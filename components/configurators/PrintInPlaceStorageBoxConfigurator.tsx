...
import { generatePrintInPlaceStorageBoxParameters } from "@/lib/engineering/makerworld";
...
const FIELD_KEYS = ["width", "depth", "height"] as const;
type FieldKey = (typeof FIELD_KEYS)[number];
type FieldValues = Record<FieldKey, string>;

const EMPTY_VALUES: FieldValues = { width: "", depth: "", height: "" };
...
export default function PrintInPlaceStorageBoxConfigurator() {
  ...
  const limits = ENGINEERING_LIMITS.printInPlaceStorageBox;
  const fieldLimits = outsideLed
    ? {
        width: limits.outside.boxWidth,
        depth: limits.outside.boxDepth,
        height: limits.outside.boxHeight,
      }
    : limits.usable;
  const labels = outsideLed
    ...
    : {
        ...
        depth: "Required usable depth",
        height: "Required usable height",
      };
  const validity = Object.fromEntries(
    FIELD_KEYS.map((key) => [
      key,
      isDimensionValid(values[key], fieldLimits[key]),
    ]),
  ...
  const result = useMemo(() => {
    ...
    return calculatePrintInPlaceStorageBox({ strategy, dimensions });
  ...
  function updateValue(key: FieldKey, value: string) {
    ...
    setValues((current) => ({ ...current, [key]: cleanedValue }));

    if (!becameValid) return;

    const nextKey = FIELD_KEYS[FIELD_KEYS.indexOf(key) + 1];
      ...
      {strategy && (
        <div className="scroll-mt-20" data-workflow-section="dimension-inputs">
          <ParameterInput
            ...
            description={
              outsideLed
                ? "Enter the physical outside dimensions of the box."
          ...
          >
            <div className="grid gap-4 sm:grid-cols-2">
              {FIELD_KEYS.map((key, index) => {
                ...
                const disabled = index > 0 && !validity[FIELD_KEYS[index - 1]];

                return (
                  <div key={key} data-workflow-field={key}>
                    <DimensionField
                      id={`print-in-place-${strategy}-${key}`}
                      label={labels[key]}
                      value={values[key]}
        ...
        <>
          ...
          >
            ...
            <dl className="mt-4 max-w-xl rounded-lg border border-neutral-200 bg-white px-4">
              ...
              />
              <ResultRow
                label={outsideLed ? "Usable height" : "Box height"}
                value={outsideLed ? result.usable.height : result.outside.height}
              />
            </dl>
          </section>

          <MakerWorldParameterPreview
            parameters={generatePrintInPlaceStorageBoxParameters(result)}
            inputOrder={["boxWidth", "boxDepth", "boxHeight"]}
            makerWorldUrl={PRODUCTS.printInPlaceStorageBox.makerWorldUrl}
